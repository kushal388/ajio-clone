...
export default function AdminProductForm({ open, onClose, onSaved, initial = {}, api }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      let res;
      if (isEdit) {
        res = await api.put(`/api/admin/products/${initial._id || initial.id}`, payload);
      } else {
        // admin does not add new products, so block
        alert("Admin cannot add new products");
        return;
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-6">
      ...
      <form
        onSubmit={submit}
        className="relative bg-white rounded-lg w-full max-w-2xl p-6 shadow-lg z-10"
      >
